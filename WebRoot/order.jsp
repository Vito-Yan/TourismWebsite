...
<script>
   ...
   function addInfo(){
      ...
      $("#info").append($("<div class='dingdan-mm' id='"+(count)+"'><div class='dingdan-x'>游客信息</div>"+
                        ...
                        "</li><li><span>*</span> 手机 <input name='tel' type='text' id='tel"+count+"' size='20' />"+
                        "</li><li><span>*</span> 身 份 证&nbsp;<input name='IdCard' type='text' id='IdCard"+count+"' size='20'/>"+
...
<c:choose>
   <c:when test="${line.teamBuy==1}">
     <li>￥<c:out value="${fn:substring(line.teamBuyPrice,0,fn:indexOf(line.teamBuyPrice,'.'))}"/>元</li>
       <input type="hidden" name="price" value="<c:out value="${fn:substring(line.teamBuyPrice,0,fn:indexOf(line.teamBuyPrice,'.'))}"/>"/>
   </c:when>
   <c:otherwise>
         <li>￥<c:out value="${fn:substring(line.price,0,fn:indexOf(line.price,'.'))}"/>元</li>
...
<div class="jine" style="display:none">应付总额：<span>5689</span>元</div>
<div class="xyb">
  <input name="button" type="image" id="button" onclick="next();" value="提交" src="<%=path%>/images/xyb.jpg" />
</div>
</div>
</form>
</div>
