...
<link href="<%=path%>/images/admin/images/skin.css" rel="stylesheet"
	type="text/css" />
...
em.error {
	background: url("<%=path%>/images/admin/images/unchecked.gif") no-repeat
		0px 0px;
	padding-left: 16px;
}

em.success {
	background: url("<%=path%>/images/admin/images/checked.gif") no-repeat
		0px 0px;
	padding-left: 16px;
}
</style>
...
<script>
	$(document).ready(function() {
		...
		$.validator.addMethod("compareTime", function(value, element) {
			var bTime = $("#bt").val(); //得到线路价格

			return value > bTime ? true : false;
...
<body>
	<form
		action="<%=path%>/AdminManagerServlet?flag=_seteam&lineID=${line.lineID}"
		enctype="multipart/form-data" method="post" id="commentForm">
		<div class="pageTop">
			<div class="page">
				<img src="<%=path%>/admin/img/coin02.png" /><span><a
					href="#">首页</a>&nbsp;-&nbsp;<a href="#">设置信息</a>&nbsp;-</span>&nbsp;设置团购信息
			</div>
		</div>
			...
			<table width="800px" align="center" height="144" border="0"
				...
				<tr>
					...
					<td width="50%" class="_text"><span>线路价格&nbsp;<input
							...
							value="${fn:substring(line.price,0,fn:indexOf(line.price,'.'))}"></span></td>
					<td><input type="submit" onclick="sub()" " value="设置团购信息">
					</td>
				</tr>
				<tr>
					...
					<td width="50%" class="_text"><span>团购价格&nbsp; <input
							type="text" name="teamprice" id="teamprice"
							value="${fn:substring(line.teamBuyPrice,0,fn:indexOf(line.teamBuyPrice,'.'))}"></span></td>


				</tr>
				<tr>
					<td width="50%"><span class="_text">开团时间&nbsp; <input
							type="text" name="beginTime" id="bt" value="${line.beginTime}"
							onclick="SelectDate(this)" readonly="readonly">
					</span></td>
					<td width="50%"><span class="_text">闭团时间&nbsp; <input
							type="text" name="endTime" id="et" value="${line.endTime}"
							onclick="SelectDate(this)" readonly="readonly">
					</span></td>
				</tr>
				<tr>
